    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" id="frm-<?=$form?>" {{$attributes->merge(['action'=> route('master-material.raw-material.store')])}} enctype="multipart/form-data">
                        @csrf
                        @if($editMode)
                        @method('PATCH')
                        <input type="hidden" id="old_kode" name="old_kode" value="{{old('old_kode')?old('old_kode'):config('constants.'.$form).'000 0000'}}">
                        @endif
    ...
    <script>
        ...
        function generateCode(){
            if (!formLoad){
                ...
                @endif
                $.ajax({
                    url         : '{{route('master-material.raw-material.generate-code')}}',
                    type        : 'get',
        ...
        document.addEventListener('DOMContentLoaded',function (){
            ...
            document.getElementById('old_kode').value = '{{$dataEdit[0]['kode']}}';
